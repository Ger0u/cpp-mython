...
private:
    runtime::ClassInstance cls_ins_;
    std::vector<std::unique_ptr<Statement>> args_;
};
...
public:
    ...
    : args_(CreateArgs(std::forward<Args>(args)...)) {
    }
...
private:
    ...
    static std::vector<std::unique_ptr<Statement>> CreateArgs() {
        return {};
    }
    
    template <typename Arg, typename... Args>
    static std::vector<std::unique_ptr<Statement>> CreateArgs(Arg&& arg, Args&&... args) {
        std::vector<std::unique_ptr<Statement>> result;
        if constexpr (sizeof...(args) > 0) {
            result = CreateArgs(std::forward<Args>(args)...);
        }
        result.emplace(result.begin(), std::forward<Arg>(arg).release());
        return result;
    }
};
